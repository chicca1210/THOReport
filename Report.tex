...
In base alla metodologia Agile, abbiamo pianificato su Trello i 5 sprint da 20 ore, e, alla fine di ognuno di essi, abbiamo fatto una sprint review.\\Dopo aver svolto l'analisi del problema abbiamo creato il Product Backlog composto da varie user story, suddivise nei 5 sprint in ordine di priorità. \\
Durante i primi incontri, ci siamo visti faccia a faccia per eseguire la raccolta dei requisiti, e discutere dell'architettura del sistema. Abbiamo impostato l'ambiente di sviluppo, compresi i repository e gli strumenti di build automatica.
In base alla raccolta dei requisiti, abbiamo scelto la parte da cui iniziare e abbiamo cominciato a lavorare.

Per la scelta dell'infrastruttura ci siamo affidati fin da subito ad AWS (Amazon Web Services) lanciando due istanze: una per la computazione (EC2) ed una per il Database (RDS). 
Durante la configurazione di queste istanze abbiamo avuto alcune difficoltà, risolte grazie al lavoro di squadra, consultando alcune guide online.
Per la scelta del middleware, ci siamo affidati a RabbitMQ, vista anche l'integrazione con i sistemi di AWS. Anche in questo caso, sebbene la configurazione iniziale non ci abbia portato via molto tempo, abbiamo riscontrato non poche difficoltà nell'impostare la comunicazione nel modo giusto tra le varie entità in gioco (Server, App Desktop e App Mobile).

Per quanto riguarda SCRUM, inizialmente abbiamo avuto difficoltà nel riuscire a stimare i tempi corretti per ogni task, ma continuando a lavorarci sopra siamo migliorati. L'ultima parte del progetto, causa indisponibilità di alcuni dei membri del team, è stata svolta dopo un periodo di pausa dovuto alla stima dei tempi sbagliata, che ci ha portato a non terminare nei tempi previsti.

L'ultimo mese in cui è stato ultimato il progetto, non sono state rispettate le regole di Scrum, dovute anche al fatto che non era facile comunicare tra noi, dato che uno dei componenti del gruppo era in erasmus in germania. Tuttavia è stato necessario procedere in modo "non consono" per garantire la consegna del progetto nei nuovi tempi previsti.
